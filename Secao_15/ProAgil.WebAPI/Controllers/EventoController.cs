    ...
    public class EventoController : ControllerBase
    {
        ...
        public async Task<IActionResult> Put(int eventoId, EventoDto model)
        {
            try
            {
                ...
                {
                    if(!model.Lotes.Any(l => l.Id == lote.Id))
                    {
                        this.IProAgilRepository.Delete(lote);
                    }
                });

                evento.RedeSociais.ForEach(rede => 
                {
                    if(!model.RedeSociais.Any(r => r.Id == rede.Id))
                    {
                        this.IProAgilRepository.Delete(rede);
                    }
                });
